...
module.exports = class OccurrenceManager {
  ...
  markBufferRangeByPattern(pattern, occurrenceType) {
    ...
    if (occurrenceType === "subword") {
      const subwordRegex = this.editor.getLastCursor().subwordRegExp()
      const cache = {}
      this.editor.scan(pattern, ({range}) => {
        const {row} = range.start
        if (!cache[row]) cache[row] = collectRangeInBufferRow(this.editor, row, subwordRegex)
        if (cache[row].some(subwordRange => subwordRange.isEqual(range))) markRange({range})
      })
    } else {
      this.editor.scan(pattern, markRange)
    }
  }

  updateEditorElement() {
    this.editor.element.classList.toggle("has-occurrence", this.hasMarkers())
  }
  ...
  buildPattern() {
    const source = this.patterns.map(regex => regex.source).join("|")
  ...
  select() {
    ...
    if (allRanges.length) {
      ...
      this.vimState.swrap.getSelections(this.editor).forEach($selection => $selection.saveProperties())
      return true
    } else {
      return false
    }
