...
module.exports = class CursorStyleManager {
  ...
  modifyCursorStyle(cursor, cursorStyle) {
    ...
    const cursorNode = this.editor.element.component.linesComponent.cursorsComponent
      .cursorNodesById[cursor.id]
    if (cursorNode) {
  ...
  getCursorBufferPositionToDisplay(selection) {
    let bufferPosition = this.vimState
      .swrap(selection)
      .getBufferPositionFor("head", {from: ["property"]})

    const editor = this.editor
    if (editor.hasAtomicSoftTabs() && !selection.isReversed()) {
      const screenPosition = editor.screenPositionForBufferPosition(
        bufferPosition.translate([0, 1]),
        {clipDirection: "forward"}
      )
      const bufferPositionToDisplay = editor
