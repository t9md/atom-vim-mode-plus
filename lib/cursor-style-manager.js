...
module.exports = class CursorStyleManager {
  ...
  refresh() {
    ...
    const blockwiseHeadCursors =
      this.vimState.submode === "blockwise"
    ...
    for (const cursor of editor.getCursors()) {
      if (blockwiseHeadCursors && !blockwiseHeadCursors.includes(cursor)) {
        this.render(cursor, {visibility: "hidden"})
      } else {
        const {column, row} = this.getCursorScreenPositionToDisplay(cursor).traversalFrom(cursor.getScreenPosition())
        this.render(cursor, {top: lineHeight * row + "px", left: column + "ch", visibility: "visible"})
      }
    }
  }
  ...
  getCursorScreenPositionToDisplay({selection}) {
    const {editor} = this.vimState
    ...
    // When following two softtab(four space each) selected, show cursor at "b" not "a"
    //         a  b c
    //         v  v v
    //   |>>>>|>>>>|>>>>|
    //
    ...
    return needClip
      ? editor.clipScreenPosition(screenPosition.translate([0, 1]), {clipDirection: "forward"}).translate([0, -1])
      : screenPosition
  }
}
