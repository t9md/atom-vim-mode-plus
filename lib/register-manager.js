...
module.exports = class RegisterManager {
  ...
  // Returns nothing.
  set(name, value) {
    if (READ_ONLY_REGISTERS_REGEX.test(name)) {
      return
    }

    if (/^[A-Z]$/.test(name)) {
      this.appendOrSet(name, value)
    }
  ...
  appendOrSet(name, value) {
    ...
    const oldValue = this.data[lowerName]
    if (oldValue) {
      this.set(lowerName, this.appendValue(oldValue, value))
    } else {
      this.set(lowerName, value)
    }
  }
