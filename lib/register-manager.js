...
module.exports = class RegisterManager {
  ...
  readClipboard(selection) {
    if (
      selection &&
      selection.editor.hasMultipleCursors() &&
      this.clipboardBySelection.has(selection)
    ) {
  ...
  get(name, selection) {
    ...
    switch (name) {
      ...
      case "+":
        ...
        break
      case "%":
        text = this.vimState.editor.getURI()
        break
      case "_":
        text = ""
        break // Blackhole always returns nothing
      default:
        const value = this.data[name.toLowerCase()]
        ;({text, type} = value != null ? value : {})
    }
    if (!type) type = this.getCopyType(text)
    return {text, type}
  }
