...
module.exports = class RegisterManager {
  ...
  getHistory() {
    if (!this.history.length) {
      ...
      this.history.push({text: text, type: this.getCopyType(text)})
    }

    const value = this.history.shift()
  ...
  saveHistory(value) {
    ...
    this.history.splice(this.vimState.getConfig("maxHistoryForSequentialPaste"))
  }
  ...
  set(name, value) {
    ...
    if (["*", "+"].includes(name)) {
      ...
      if (this.vimState.getConfig("enableSequentialPaste")) {
