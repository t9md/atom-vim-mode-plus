...
module.exports = class VimState {
  ...
  setIgnoreTextInput (state) {
    const component = this.editorElement.component
    // Component is not necessary avaiable see #98.
    if (component) {
      if (component.setIgnoreTextInput) {
        component.setIgnoreTextInput(state)
      } else {
        component.setInputEnabled(!state)
      }
    }
  }
  ...
  activate (newMode, newSubmode = null) {
    ...
    else if (newMode === 'insert') this.setIgnoreTextInput(false)
