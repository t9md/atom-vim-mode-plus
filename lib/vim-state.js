...
module.exports = class VimState {
  ...
  static registerCommandFromSpec(klassName, {scope = "atom-text-editor", name, prefix, getClass, loader}) {
    const commandName = name || prefix + ":" + dasherize(klassName)
    if (loader) getClass = klassName => this.loadPluginLoader(loader)[klassName]
    const dispatcher = this.getDispatcherDispatchBy(() => getClass(klassName))
    return atom.commands.add(scope, commandName, dispatcher)
  }
