...
const RENAMED_PARAMS = [{oldName: "keepColumnOnSelectTextObject", newName: "stayOnSelectTextObject"}]
...
class Settings {
  ...
  migrateRenamedParams() {
    ...
    for (const {oldName, newName, inverseValue} of RENAMED_PARAMS) {
      const oldValue = this.get(oldName)
      if (oldValue != null) {
        const newValue = inverseValue ? !oldValue : oldValue
        ...
        if (inverseValue) s += " with value **inversed**".messages.push(s)
        messages.push(s)
      }
    }
    ...
    const message = `${this.scope}: Config options renamed and auto-migrated.\n${messages.join("\n")}`
