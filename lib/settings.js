...
module.exports = new Settings("vim-mode-plus", {
  ...
  findCharsMax: {
    ...
    minimum: 1,
    description:
      "Auto confirm when find( `f` )'s input reaches this lenth.<br>Increasing this number greatly reduces the possible matches, but you now need confirm( `enter` ) for shorter input.<br><br>[Hint]: Set big number(such as `100`) with also eanabling `findConfirmByTimeout`, you now **always** land(confirm) by timeout. Thus you now can expect **FIXED-TIMING-GAP** against your input(in \"f > input > timeout > landbb\" flow).<br><br>See also \"`keymap `;` to confirm `find` motion`\" configuration to make manual confirmation easy.",
  },
  findConfirmByTimeout: {
    default: 0,
    description:
