  ...
  if (editor.softTabs) {
    ...
    const column =
      cursor.goalColumn != null
        ? cursor.goalColumn
        : translateBufferPositionOnHardTabEditor(editor, cursor.getBufferPosition(), true).column

    cursor.setBufferPosition(translateBufferPositionOnHardTabEditor(editor, [row, column], false), options)
    cursor.goalColumn = column
  }
}

function translateBufferPositionOnHardTabEditor (editor, point, expandTab) {
  const {row, column} = Point.fromObject(point)
  ...
  let newColumn = 0
  if (expandTab) {
    for (const char of lineText.slice(0, column)) {
    ...
    for (const char of lineText) {
      ...
      if (traversedColumn >= column) {
        if (traversedColumn > column && newColumn > 0) {
          newColumn--
        }
        break
      }
  ...
  return new Point(row, newColumn)
}
