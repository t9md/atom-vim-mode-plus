...
function getBeginningOfWordBufferPosition(editor, from, {wordRegex} = {}) {
  const found = findInEditor(editor, "backward", wordRegex, {from, allowNextLine: false}, ({range}) => {
    if (range.start.isLessThan(from) && range.end.isGreaterThanOrEqual(from)) {
      return range.start
    }
  })
  return found || from
...
function getEndOfWordBufferPosition(editor, from, {wordRegex} = {}) {
  const found = findInEditor(editor, "forward", wordRegex, {from, allowNextLine: false}, ({range}) => {
    if (range.start.isLessThanOrEqual(from) && range.end.isGreaterThan(from)) {
      return range.end
    }
  })
  return found || from
...
function scanEditor(editor, direction, regex, options, fn) {
  ...
  const {allowNextLine = true, skipEmptyRow, skipWhiteSpaceOnlyRow} = options
