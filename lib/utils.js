...
function getUnionedRangeForFoldsIntersectsScreenRow(editor, screenRow) {
  ...
  if (!markers.length) {
    throw new Error("getUnionedRangeForFoldsIntersectsScreenRow() called for non-folded screenRow!")
  }
  return markers.map(marker => marker.getRange()).reduce((range, unionedRange) => unionedRange.union(range))
}

// Return smalest row of folded row(multiple folds maybe exists in single screen row) if row is folded.
function getFoldStartRowForRow(editor, row) {
  return editor.isFoldedAtBufferRow(row)
    ? getUnionedRangeForFoldsIntersectsScreenRow(editor, editor.screenRowForBufferRow(row)).start.row
    : row
}

// Return largest row of folded row(multiple folds maybe exists in single screen row) if row is folded.
function getFoldEndRowForRow(editor, row) {
  return editor.isFoldedAtBufferRow(row)
    ? getUnionedRangeForFoldsIntersectsScreenRow(editor, editor.screenRowForBufferRow(row)).end.row
    : row
}
...
module.exports = {
  ...
  getUnionedRangeForFoldsIntersectsScreenRow,
