...
function getFoldStartRowForRow(editor, bufferRow) {
  return editor.isFoldedAtBufferRow(bufferRow)
    ? editor.bufferRowForScreenRow(editor.screenRowForBufferRow(bufferRow) - 1) + 1
    : bufferRow
}

// Return endRow of fold if row was folded or just return passed row.
function getFoldEndRowForRow(editor, bufferRow) {
  return editor.isFoldedAtBufferRow(bufferRow)
    ? editor.bufferRowForScreenRow(editor.screenRowForBufferRow(bufferRow) + 1) - 1
    : bufferRow
}
