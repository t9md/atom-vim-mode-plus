...
module.exports = class ModeManager {
  ...
  activate(newMode, newSubmode = null) {
    ...
    if (newMode === "visual" && this.submode && this.submode === newSubmode) {
      newMode = "normal"
      newSubmode = null
    }
    ...
    if (this.mode === "visual") {
      ...
      if (this.vimState.hasProp("swrap")) this.vimState.swrap.clearProperties(this.editor)
    }
    ...
    if (this.mode === "visual" || this.vimState.hasProp("cursorStyleManager")) {
  ...
  activateReplaceMode() {
    ...
    disposable.add(
      ...
      new Disposable(() => (this.replacedCharsBySelection = null))
    )
    return disposable
  }

  getReplacedCharForSelection(selection) {
    const replacedChars = this.replacedCharsBySelection.get(selection)
    return replacedChars ? replacedChars.pop() : undefined
  }
