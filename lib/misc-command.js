...
class ActivateNormalModeOnce extends InsertMode {
  execute() {
    const thisCommandName = this.constructor.getCommandName()
    ...
    let disposable = atom.commands.onDidDispatch(({type}) => {
      if (type === thisCommandName) return
...
class InsertLastInserted extends InsertMode {
  static description = "Insert text inserted in latest insert-mode(*i_CTRL-A* of pure Vim)."
  execute() {
...
class CopyFromLineAbove extends InsertMode {
  static description = "Insert character of same-column of above line(*i_CTRL-Y* of pure Vim)."
  rowDelta = -1
...
class CopyFromLineBelow extends CopyFromLineAbove {
  static description = "Insert character of same-column of above line.(*i_CTRL-E* of pure Vim)."
  rowDelta = +1
}
