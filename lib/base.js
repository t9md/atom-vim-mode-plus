...
class Base {
  ...
  static buildCommandTable() {
    ...
    for (const klass of Object.values(CLASS_REGISTRY)) {
      const file = klass.vmpFileToLoad
      if (klass.isCommand()) {
        const spec = {file, name: klass.getCommandName()}
        if (klass.commandScope != null) spec.scope = klass.commandScope // avoid scope appear if falthy value
        commandTable[klass.name] = spec
      } else {
        commandTable[klass.name] = {file}
      }
    }
    return commandTable
  }
  ...
  static init(getEditorState) {
    ...
    return Object.keys(this.commandTable)
      ...
      .map(name => this.registerCommandFromSpec(name, this.commandTable[name], true))
  }
  ...
  static registerCommand() {
    return this.registerCommandFromSpec(
      this.name,
      {
  ...
  // TODO: Remove this and `skipMigration` flag after a vmp v1.18 was out.
  static migrateOldCommandSpec(spec) {
    const changesInName = [
      {oldName: "commandScope", newName: "scope"},
      {oldName: "commandPrefix", newName: "prefix"},
      {oldName: "commandName", newName: "name"},
    ]
    for (const {oldName, newName} of changesInName) {
      spec[newName] = spec[oldName]
      delete spec[oldName]
    }
    return spec
  }

  static registerCommandFromSpec(klassName, spec, skipMigration) {
    // TODO: Remove this and `skipMigration` flag after a vmp v1.18 was out.
    if (!skipMigration) spec = this.migrateOldCommandSpec(spec)

    const {scope = "atom-text-editor", prefix, name, getClass} = spec
    const {getEditorState} = this
    return atom.commands.add(scope, name || this.getCommandName(prefix, klassName), function(event) {
      // Folloiwng guard is removed from vmp v1.18.0 since I believe this is NOT necessary.
      // But will keep this comment as memorial of removal since I'm not 100% confident my decision.
      ...
      if (vimState) vimState.operationStack.run(getClass ? getClass(klassName) : klassName)
