...
class Base {
  ...
  static async writeCommandTableOnDisk() {
    ...
    const needUpdateCommandTable = !_.isEqual(require("./command-table.json"), commandTable)
    const needUpdateFileTable = !_.isEqual(require("./file-table.json"), fileTable)
    ...
    if (needUpdateFileTable) {
      await writeJSON("file-table.json", fileTable)
  ...
  static buildCommandTableAndFileTable() {
    ...
    for (const file of filesToLoad) {
      for (const klass of Object.values(require(file))) {
        ...
        if (klass.isCommand()) {
          commandTable.push(klass.getCommandNameWithoutPrefix())
        }
      }
    }
  ...
  static init() {
    return require("./command-table.json").map(name =>
      this.registerCommandFromSpec(null, {name: "vim-mode-plus:" + name})
    )
  }

  static getClass(name) {
    if (!(name in this.classTable)) {
      if (!FILE_TABLE) FILE_TABLE = require("./file-table.json")
