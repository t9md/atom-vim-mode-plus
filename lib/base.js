...
function loadVmpOperationFile(filename) {
  const newModules = ["./operator", "./motion", "./motion-search"]
  if (newModules.includes(filename)) {
    const klasses = require(filename)
    for (const name in klasses) {
      const klass = klasses[name]
      klass.file = filename
      klass.register(true, false)
    }
    return
  }
...
class Base {
  ...
  static register(command = true, setfile = true) {
    if (!this.hasOwnProperty("command")) {
      this.command = command
    }
    if (this.name === "Operator") {
      console.log("Operator ====================")
      console.log(this.hasOwnProperty("command"), this.command, this.isCommand())
    }
    if (setfile) {
