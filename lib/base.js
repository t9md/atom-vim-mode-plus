...
class Base {
  ...
  //   - Save command table to "./command-table.json".
  //   - Save file table to "./file-table.json".
  //
  ...
  static async writeCommandTableOnDisk() {
    ...
    const _ = _plus()
    const needUpdateCommandTable = !_.isEqual(this.commandTable, commandTable)
    const needUpdateFileTable = !_.isEqual(this.fileTable, fileTable)
    ...
    const writeJSON = (baseName, object) => {
      return atom.workspace.open(join(__dirname, baseName), {activatePane: false, activateItem: false}).then(editor => {
        editor.setText(JSON.stringify(object))
        return editor.save()
      })
    }

    if (needUpdateCommandTable) {
      await writeJSON("command-table.json", commandTable)
      atom.notifications.addInfo("Updated commandTable", {dismissable: true})
    }
    if (needUpdateFileTable) {
      await writeJSON("file-table.json", FILE_TABLE)
      atom.notifications.addInfo("Updated fileTable", {dismissable: true})
    }
  }
  ...
  static buildCommandTableAndFileTable() {
    ...
    for (const file of filesToLoad) {
      for (const klass of Object.values(require(file))) {
        ...
        if (name in fileTable) {
          throw new Error(`Duplicate operation class ${name} in "${file}" and "${fileTable[name]}"`)
       }

        fileTable[name] = file

        if (klass.isCommand()) {
          const spec = {name: klass.getCommandName()}
  ...
  static get fileTable() {
    return this.__fileTable || (this.__fileTable = require("./file-table.json"))
  }

  static getClass(name) {
    if (!(name in this.classTable)) {
      if (atom.inDevMode() && settings.get("debug")) {
        console.log(`lazy-require: ${this.fileTable[name]} for ${name}`)
      }
      Object.values(require(this.fileTable[name])).forEach(klass => klass.register())
    }
