...
class Base {
  ...
  focusInputPromisified(options) {
    const onChange = input => this.vimState.hover.set(input)
    return new Promise((onConfirm, onCancel) => {
      this.vimState.focusInput(Object.assign(options, {onConfirm, onCancel, onChange}))
    })
  }
  ...
  readCharPromisified() {
    // Resolve with undefined when cancelled
