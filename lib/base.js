...
class Base {
  ...
  //   - buildCommandTable()
  //   - Write commandTable to disk as CSON(CoffeeScript verison of JSON) form.
  //
  // 2. [On atom/vmp startup]
  //   - Load commandTable from CSON.
  ...
  static async writeCommandTableOnDisk() {
    ...
    const filePath = path.join(__dirname, "command-table.coffee")
  ...
  static registerCommandFromSpec(name, spec) {
    const {commandScope = "atom-text-editor", commandPrefix, commandName, getClass} = spec
    const {getEditorState} = this
    return atom.commands.add(commandScope, commandName || this.getCommandName(commandPrefix, name), function(event) {
      ...
      if (vimState) vimState.operationStack.run(getClass ? getClass(name) : name)
