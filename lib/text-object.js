...
class TextObject extends Base {
  ...
  static deriveClass(innerAndA, innerAndAForAllowForwarding) {
    ...
    const result = {}
    const klasses = []
    if (innerAndA) klasses.push(this.generateClass(false), this.generateClass(true))
    if (innerAndAForAllowForwarding) klasses.push(this.generateClass(false, true), this.generateClass(true, true))
    klasses.forEach(klass => (result[klass.name] = klass))
    return result
  }
...
const {AWord, InnerWord} = Word.deriveClass("InnerAndA")

class WholeWord extends Word {
  wordRegex = /\S+/
}
const {AWholeWord, InnerWholeWord} = WholeWord.deriveClass("InnerAndA")

// Just include _, -
class SmartWord extends Word {
  wordRegex = /[\w-]+/
}
const {ASmartWord, InnerSmartWord} = SmartWord.deriveClass("InnerAndA")
...
const {ASubword, InnerSubword} = Subword.deriveClass("InnerAndA")
...
const {AAnyPair, InnerAnyPair} = AnyPair.deriveClass("InnerAndA")
...
const {AAnyPairAllowForwarding, InnerAnyPairAllowForwarding} = AnyPairAllowForwarding.deriveClass("InnerAndA")
...
const {AAnyQuote, InnerAnyQuote} = AnyQuote.deriveClass("InnerAndA")
...
const {ADoubleQuote, InnerDoubleQuote} = DoubleQuote.deriveClass("InnerAndA")

class SingleQuote extends Quote {
  pair = ["'", "'"]
}
const {ASingleQuote, InnerSingleQuote} = SingleQuote.deriveClass("InnerAndA")

class BackTick extends Quote {
  pair = ["`", "`"]
}
const {ABackTick, InnerBackTick} = BackTick.deriveClass("InnerAndA")

class CurlyBracket extends Pair {
  pair = ["{", "}"]
}
const {ACurlyBracket, InnerCurlyBracket} = CurlyBracket.deriveClass("InnerAndA", "InnerAndAForAllowForwarding")

class SquareBracket extends Pair {
  pair = ["[", "]"]
}
const {ASquareBracket, InnerSquareBracket} = SquareBracket.deriveClass("InnerAndA", "InnerAndAForAllowForwarding")

class Parenthesis extends Pair {
  pair = ["(", ")"]
}
const {AParenthesis, InnerParenthesis} = Parenthesis.deriveClass("InnerAndA", "InnerAndAForAllowForwarding")

class AngleBracket extends Pair {
  pair = ["<", ">"]
}
const {AAngleBracket, InnerAngleBracket} = AngleBracket.deriveClass("InnerAndA", "InnerAndAForAllowForwarding")
...
const {ATag, InnerTag} = Tag.deriveClass("InnerAndA")
...
const {AParagraph, InnerParagraph} = Paragraph.deriveClass("InnerAndA")
...
const {AIndentation, InnerIndentation} = Indentation.deriveClass("InnerAndA")
...
const {AComment, InnerComment} = Comment.deriveClass("InnerAndA")
...
const {ACommentOrParagraph, InnerCommentOrParagraph} = CommentOrParagraph.deriveClass("InnerAndA")
...
const {AFold, InnerFold} = Fold.deriveClass("InnerAndA")
...
const {AFunction, InnerFunction} = Function.deriveClass("InnerAndA")
...
const {AArguments, InnerArguments} = Arguments.deriveClass("InnerAndA")
...
const {ACurrentLine, InnerCurrentLine} = CurrentLine.deriveClass("InnerAndA")
...
const {AEntire, InnerEntire} = Entire.deriveClass("InnerAndA")
...
const {ALatestChange, InnerLatestChange} = LatestChange.deriveClass("InnerAndA")
...
const {APersistentSelection, InnerPersistentSelection} = PersistentSelection.deriveClass("InnerAndA")
...
const {AVisibleArea, InnerVisibleArea} = VisibleArea.deriveClass("InnerAndA")
