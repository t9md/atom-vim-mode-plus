...
class Function extends TextObject {
  ...
  getRange(selection) {
    ...
    const parameterEndRegex = this.getFunctionParameterEndRegex(editor.getGrammar())
    ...
    while (foldRanges.length) {
      ...
      if (isIncludeFunctionScopeForRow(range.start.row)) {
        ...
        const isIndependentFunctionBodyStart = row =>
          new RegExp("^\\s*" + parameterEndRegex.source).test(editor.lineTextForBufferRow(row))
        if (isIndependentFunctionBodyStart(range.start.row) && isIncludeFunctionScopeForRow(previousRow)) {
