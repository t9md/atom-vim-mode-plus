...
class TextObject extends Base {
  ...
  static register(...args) {
    if (this.hasOwnProperty("derives")) {
      this.command = false
    }
    super.register(...args)

    if (this.hasOwnProperty("derives")) {
      for (const kind of this.derives) {
        if (kind === "InnerAndA") {
          this.generateClass(`A${this.name}`, false)
          this.generateClass(`Inner${this.name}`, true)
        } else if (kind === "InnerAndAForAllowForwarding") {
...
class CurlyBracket extends Pair {
  static derives = ["InnerAndA", "InnerAndAForAllowForwarding"]
  pair = ["{", "}"]
}
