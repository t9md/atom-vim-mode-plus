...
Word.initClass(false)
Word.deriveInnerAndA()

class WholeWord extends Word {
  wordRegex = /\S+/
}
WholeWord.initClass(false)
WholeWord.deriveInnerAndA()

// Just include _, -
class SmartWord extends Word {
  static description = "A word that consists of alphanumeric chars(`/[A-Za-z0-9_]/`) and hyphen `-`"
  wordRegex = /[\w-]+/
}
SmartWord.initClass(false)
...
AnyPair.initClass(false)
AnyPair.deriveInnerAndA()

class AnyPairAllowForwarding extends AnyPair {
  static description = "Range surrounded by auto-detected paired chars from enclosed and forwarding area"
...
AnyPairAllowForwarding.initClass(false)
...
class CommentOrParagraph extends TextObject {
  ...
  getRange(selection) {
    ...
    for (const klass of ["Comment", "Paragraph"]) {
      const range = this.new(klass).assign({inner}).getRange(selection)
      if (range) return range
    }
  }
}
CommentOrParagraph.initClass(false)
...
PersistentSelection.initClass(false)
