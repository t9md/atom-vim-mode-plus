...
class MoveToPreviousEndOfWord extends WordMotion {
  ...
  moveCursor(cursor) {
    ...
    this.moveCursorCountTimes(cursor, () => {
      ...
      const fromPoint = this.editor.clipBufferPosition(cursor.getBufferPosition().translate([0, +1]))
      cursor.setBufferPosition(this.findPreviousEndOfWord(fromPoint, regex))
    })
  }
...
class MoveToNextSentence extends Motion {
  ...
  getNextStartOfSentence(from) {
    let foundPoint
    this.scanForward(this.sentenceRegex, {from}, ({range, match, stop}) => {
      if (match[1] != null) {
  ...
  getPreviousStartOfSentence(from) {
    let foundPoint
    this.scanBackward(this.sentenceRegex, {from}, ({range, match, stop}) => {
      if (match[1] != null) {
        const [startRow, endRow] = [range.start.row, range.end.row]
