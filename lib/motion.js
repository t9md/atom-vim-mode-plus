...
class Scroll extends Motion {
  ...
  execute() {
    const firstVisibleRow = this.editor.getFirstVisibleScreenRow()
    const lastVisibleRow = this.utils.limitNumber(this.editor.getLastVisibleScreenRow(), {max: this.getVimLastScreenRow()})
    this.amountOfRowsToScroll = Math.ceil(this.amountOfPage * Math.floor(lastVisibleRow - firstVisibleRow) * this.getCount())
