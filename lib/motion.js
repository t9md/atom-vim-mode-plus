...
class MotionByFold extends Motion {
  ...
  execute () {
    ...
    if (this.direction === 'previous') {
      this.foldRanges.sort((rangeA, rangeB) => rangeB.compare(rangeA))
    }
    super.execute()
  }

  findRowBy (cursor, fn) {
    for (const foldRange of this.foldRanges) {
      const row = foldRange[this.which].row
      if (this.direction === 'previous') {
        if (row < cursor.getBufferRow() && fn(foldRange)) {
          return row
        }
      } else {
        if (row > cursor.getBufferRow() && fn(foldRange)) {
...
class MotionByFoldWithSameIndent extends MotionByFold {
  ...
  findRow (cursor) {
    ...
    return this.findRowBy(cursor, foldRange => isEqualIndentLevel(foldRange))
  }
}
...
class MoveToPreviousFunction extends MotionByFold {
  ...
  direction = 'previous'
  findRow (cursor) {
    return this.findRowBy(cursor, foldRange => {
      return this.utils.isIncludeFunctionScopeForRow(this.editor, foldRange.start.row)
    })
  }
}
