...
class MoveToTopOfScreen extends Motion {
  ...
  getScreenRow() {
    ...
    const scrolloff = this.isAsTargetExceptSelectInVisualMode() ? 0 : this.scrolloff

    if (this.where === "top") {
      const offset = firstVisibleRow === 0 ? 0 : scrolloff
      ...
      const offset = lastVisibleRow === this.getVimLastScreenRow() ? 0 : scrolloff + 1
