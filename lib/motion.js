...
class MoveUp extends Motion {
  ...
  getBufferRow(row) {
    ...
    if (this.direction === "up") {
      row = this.wrap && row === min ? max : this.utils.limitNumber(row - 1, {min})
      ...
      return this.wrap && row >= max ? min : this.utils.limitNumber(row + 1, {max})
    }
  }
...
class MoveToNextSentence extends Motion {
  ...
  moveCursor(cursor) {
    this.moveCursorCountTimes(cursor, () => {
      ...
      const point = this.direction === "next"
