...
class Find extends Motion {
    ...
    if (charsMax > 1) {
      ...
      options = {
        ...
        onChange: preConfirmedChars => {
          ...
          return this.highlightTextInCursorRows(this.preConfirmedChars, "pre-confirm")
        },
        onCancel: () => {
  ...
  repeatIfNecessary() {
    ...
    const isSequentialExecution = ((needle = this.vimState.operationStack.getLastCommandName()),
    ["Find", "FindBackwards", "Till", "TillBackwards"].includes(needle))
...
class MoveToMark extends Motion {
  static initClass() {
    ...
    // set when instatntiated via vimState::moveToMark()
  }
...
class MoveToMarkLine extends MoveToMark {
  ...
  getPoint() {
    let point
    if ((point = super.getPoint(...arguments))) {
