...
class MoveToRelativeLineMinimumTwo extends MoveToRelativeLine {
  static command = false
  getCount() {
    return this.utils.limitNumber(super.getCount(), {min: 2})
  }
}
...
class MoveToTopOfScreen extends Motion {
  ...
  getScreenRow() {
    ...
    if (this.name === "MoveToTopOfScreen") {
      ...
      return limitNumber(firstVisibleRow + count, {min: firstVisibleRow + offset, max: lastVisibleRow})
      ...
      const count = this.getCount() - 1
      return limitNumber(lastVisibleRow - count, {min: firstVisibleRow, max: lastVisibleRow - offset})
    }
  }
}
