...
CurrentSelection.initClass(false)
...
MoveRightBufferColumn.initClass(false)
...
class MoveUp extends Motion {
  ...
  getBufferRow(row) {
    ...
    return this.editor.isFoldedAtBufferRow(row) ? getLargestFoldRangeContainsBufferRow(this.editor, row).start.row : row
  }
...
class MoveUpToEdge extends Motion {
  static description = "Move cursor up to **edge** char at same-column"
  wise = "linewise"
  jump = true
  direction = "up"
  moveCursor(cursor) {
  ...
  getScanRows({row}) {
    ...
    if (this.direction === "up") {
      return getList(validRow(row - 1), 0, true)
    } else if (this.direction === "down") {
      return getList(validRow(row + 1), this.getVimLastScreenRow(), true)
    }
  }
...
class MoveDownToEdge extends MoveUpToEdge {
  static description = "Move cursor down to **edge** char at same-column"
  direction = "down"
}
...
class MoveToNextWord extends Motion {
  ...
  getPoint(regex, from) {
    ...
    if (found) {
      ...
      return pointIsAtEndOfLineAtNonEmptyRow(this.editor, point) && !point.isEqual(vimEOF)
        ? point.traverse([1, 0])
        : point
    } else {
...
MoveToScreenColumn.initClass(false)
...
MoveToRelativeLine.initClass(false)

class MoveToRelativeLineMinimumOne extends MoveToRelativeLine {
  getCount(...args) {
    return limitNumber(super.getCount(...args), {min: 1})
  }
}
MoveToRelativeLineMinimumOne.initClass(false)
...
class Till extends Find {
  offset = 1
  getPoint(...args) {
    this.point = super.getPoint(...args)
...
class MoveToPreviousFoldStart extends Motion {
  static description = "Move to previous fold start"
  wise = "characterwise"
  which = "start"
  direction = "prev"
...
class MoveToNextFoldStart extends MoveToPreviousFoldStart {
  static description = "Move to next fold start"
  direction = "next"
}
MoveToNextFoldStart.initClass()

class MoveToPreviousFoldStartWithSameIndent extends MoveToPreviousFoldStart {
  static description = "Move to previous same-indented fold start"
  detectRow(cursor) {
    const baseIndentLevel = this.getIndentLevelForBufferRow(cursor.getBufferRow())
    return this.getScanRows(cursor).find(row => this.getIndentLevelForBufferRow(row) === baseIndentLevel)
  }
}
...
class MoveToPreviousFunction extends MoveToPreviousFoldStart {
  static description = "Move to previous function"
  direction = "prev"
  detectRow(cursor) {
...
class MoveToNextFunction extends MoveToPreviousFunction {
  static description = "Move to next function"
  direction = "next"
}
...
MoveToPositionByScope.initClass(false)

class MoveToPreviousString extends MoveToPositionByScope {
  static description = "Move to previous string(searched by `string.begin` scope)"
  direction = "backward"
  scope = "string.begin"
}
...
class MoveToNextString extends MoveToPreviousString {
  static description = "Move to next string(searched by `string.begin` scope)"
  direction = "forward"
}
MoveToNextString.initClass()

class MoveToPreviousNumber extends MoveToPositionByScope {
  static description = "Move to previous number(searched by `constant.numeric` scope)"
  direction = "backward"
...
class MoveToNextNumber extends MoveToPreviousNumber {
  static description = "Move to next number(searched by `constant.numeric` scope)"
  direction = "forward"
}
...
class MoveToNextOccurrence extends Motion {
  ...
  execute() {
    this.ranges = sortRanges(this.vimState.occurrenceManager.getMarkers().map(marker => marker.getBufferRange()))
    super.execute()
  }
