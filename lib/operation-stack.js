...
module.exports = class OperationStack {
  ...
  constructor(vimState) {
    ...
    this.subscriptions.add(this.vimState.onDidDestroy(() => this.destroy()))
  ...
  process() {
    ...
    if (top.isComplete()) {
      ...
      const commandName =
        typeof top.constructor.getCommandNameWithoutPrefix === "function"
          ? top.constructor.getCommandNameWithoutPrefix()
          : undefined
      if (commandName) {
  ...
  buildCountString() {
    [this.count["normal"], this.count["operator-pending"]]
