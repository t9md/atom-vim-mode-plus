...
module.exports = class OperationStack {
    ...
    try {
      ...
      if (type === "object") {
        ...
        if (stackTop && stackTop.constructor === klass) {
          ...
          if (properties) {
            // FIXME: wanna remove properties arg on run() used in vim-mode-plus-ex-mode
            operation = new klass(this.vimState).assign(properties)
          } else {
            operation = new klass(this.vimState)
          }
        }
      }
  ...
  runRecordedMotion(key, {reverse = false} = {}) {
    ...
    recorded.rebindVimState(this.vimState)
