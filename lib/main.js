...
function getPaneUtils() {
  if (!paneUtils) paneUtils = require("./pane-utils")
  return paneUtils
}
...
module.exports = {
  ...
  activate() {
    ...
    this.subscribe(...Base.init(getEditorState))
    ...
    if (atom.inDevMode()) {
      ...
      this.subscribe(this.developer.init(getEditorState))
    }

    this.subscribe(
      this.observeAndWarnVimMode(),
      atom.workspace.observeTextEditors(editor => {
        if (!editor.isMini()) this.createVimState(editor)
      })
    )
    ...
    this.subscribe(...settings.observeConditionalKeymaps())

    if (settings.get("debug") && this.developer) {
  ...
  registerCommands() {
    this.subscribe(
      atom.commands.add("atom-text-editor:not([mini])", {
        ...
        "vim-mode-plus:clear-persistent-selection": () => this.clearPersistentSelectionForEditors(),
      }),
      atom.commands.add("atom-workspace", {
        "vim-mode-plus:maximize-pane": () => this.maximizePane(),
        "vim-mode-plus:equalize-panes": () => this.equalizePanes(),
        "vim-mode-plus:exchange-pane": () => this.exchangePane(),
        "vim-mode-plus:move-pane-to-very-top": () => this.movePaneToVery("top"),
        "vim-mode-plus:move-pane-to-very-bottom": () => this.movePaneToVery("bottom"),
        "vim-mode-plus:move-pane-to-very-left": () => this.movePaneToVery("left"),
        "vim-mode-plus:move-pane-to-very-right": () => this.movePaneToVery("right"),
      })
    )
  },
  ...
  clearPersistentSelectionForEditors() {
    for (const editor of atom.workspace.getTextEditors()) {
      this.getEditorState(editor).clearPersistentSelections()
    }
  },

  // atom-workspace commands
  exchangePane() {
    getPaneUtils().exchangePane()
  },
  ...
  maximizePane() {
    if (this.maximizePaneDisposable) {
      ...
      this.maximizePaneDisposable = getPaneUtils().maximizePane()
    }
  },
  ...
  equalizePanes() {
    getPaneUtils().equalizePanes()
  },

  movePaneToVery(direction) {
    getPaneUtils().movePaneToVery(direction)
  },
  ...
  registerVimStateCommands() {
    ...
    this.subscribe(atom.commands.add("atom-text-editor:not([mini])", bindToVimState(commands)))
  },
  ...
  consumeStatusBar(statusBar) {
    ...
    this.subscribe(new Disposable(() => statusBarManager.detach()))
  },

  consumeDemoMode(demoModeService) {
    if (!DemoModeSupport) DemoModeSupport = require("./demo-mode-support")
    this.demoModeSupport = new DemoModeSupport(demoModeService)
  },

  createVimState(editor) {
    const vimState = new VimState(editor, this.getStatusBarManager(), globalState)
    this.emitter.emit("did-add-vim-state", vimState)
  },

  createVimStateIfNecessary(editor) {
    if (!VimState.has(editor)) this.createVimState(editor)
  },
