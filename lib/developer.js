...
module.exports = new class Developer {
  init() {
    return atom.commands.add("atom-text-editor", {
      ...
      "vim-mode-plus:write-command-table-on-disk": () => Base.writeCommandTableOnDisk(),
  ...
  getCommandSpecs() {
    ...
    const {commandTable} = Base.buildCommandTableAndFileTable()

    return Object.values(Base.getClassTable())
      .filter(klass => klass.isCommand())
      .map(klass => commandSpecForClass(klass))
    ...
    function commandSpecForClass(klass) {
      ...
      const kind = klass.operationKind
      ...
      return {name, commandName, kind, keymap}
    }
  }
